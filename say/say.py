...
log = logging.getLogger("red.mr42-cogs.say")
...
class Say(commands.Cog):
	...
	async def say(
		...
		if not channel.permissions_for(ctx.guild.me).send_messages:
			if channel != ctx.channel:
				await ctx.send(error(_("I am not allowed to send messages in {channel}").format(channel=channel.mention)), delete_after=2)
			else:
				await ctx.author.send(error(_("I am not allowed to send messages in {channel}").format(channel=channel.mention)))
				# If this fails then fuck the command author
			return

		if files and not channel.permissions_for(ctx.guild.me).attach_files:
			try:
				await ctx.send(error(_("I am not allowed to upload files in {channel}").format(channel=channel.mention)), delete_after=2)
			except discord.errors.Forbidden:
				await ctx.author.send(error(_("I am not allowed to upload files in {channel}").format(channel=channel.mention)), delete_after=15)
			return
		...
		try:
			...
			with suppress(discord.errors.HTTPException):
				await ctx.send(error(_("An error occured when sending the message.")))
	...
	@checks.admin_or_permissions(administrator=True)
	async def _sayautodelete(self, ctx: commands.Context, channel: Optional[discord.TextChannel], delete_delay: int, *, text: str = ""):
	...
	@checks.admin_or_permissions(administrator=True)
	async def _saydelete(self, ctx: commands.Context, channel: Optional[discord.TextChannel], *, text: str = ""):
		...
		try:
			...
			try:
				await ctx.send(error(_("Not enough permissions to delete messages.")), delete_after=2)
			except discord.errors.Forbidden:
				await ctx.author.send(error(_("Not enough permissions to delete messages.")), delete_after=15)
	...
	async def _saymention(self, ctx: commands.Context, channel: Optional[discord.TextChannel], *, text: str = ""):
		...
		non_mentionable_roles = [bold(x.name) for x in role_mentions if x.mentionable is False]
		if not channel.permissions_for(guild.me).mention_everyone:
			if non_mentionable_roles:
				await ctx.send(error(
                    ...
                    ).format(roles=humanize_list(non_mentionable_roles))
				))
				return
			if mention_everyone:
				return await ctx.send(error(_("I don't have the permission to mention everyone.")))
		if not channel.permissions_for(ctx.author).mention_everyone:
			...
			if mention_everyone:
				return await ctx.send(error(_("You don't have the permission yourself to do mass mentions.")))
	...
	async def _interact(self, ctx: commands.Context, channel: discord.TextChannel = None):
		...
		if channel is None:
			if isinstance(ctx.channel, discord.DMChannel):
				await ctx.send(warning(_("You need to give a channel to enable this in DM. You can give the channel ID too.")))
				return
			else:
				channel = ctx.channel

		if ctx.author in self.interaction:
			await ctx.send(warning(_("A session is already running.")))
			return
		...
		while True:
			...
			try:
				...
				await ctx.author.send(_("Request timed out. Session closed"))
			...
			elif (message.channel != channel or message.author == channel.guild.me or message.author == ctx.author):
				pass
	...
	async def slash_say(
		...
		if not message and not file:
			await interaction.response.send_message(_("You cannot send an empty message."), ephemeral=True)
			return

		if not channel.permissions_for(guild.me).send_messages:
			await interaction.response.send_message(_("I don't have the permission to send messages there."), ephemeral=True)
			return
		if file and not channel.permissions_for(guild.me).attach_files:
			await interaction.response.send_message(_("I don't have the permission to upload files there."), ephemeral=True)
			return
		...
		try:
			...
			await interaction.response.send_message(_("An error occured when sending the message."), ephemeral=True)
			log.error(
	...
	async def on_reaction_add(self, reaction, user):
		if user in self.interaction and isinstance(reaction.message.channel, discord.DMChannel):
